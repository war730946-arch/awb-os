...
async function main() {
  ...
  if (!user) {
    ...
    console.log('Created admin user: admin@awb-os.com / Admin@123456\n');
  } else {
  ...
  if (businesses.length === 0) {
    business = await db.createBusiness({
      ...
      name: 'My Business',
      type: 'general',
      phone_number: '923281146929',
      description: 'A general business providing quality services to customers.',
      location: 'Main City'
    });
    console.log('Created business for phone: 923281146929\n');
  } else {
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      console.log(`\n✅ WhatsApp CONNECTED! Bot is running.\nSend any message to ${business.phone_number} on WhatsApp.\n`);
